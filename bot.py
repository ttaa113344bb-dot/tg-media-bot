...
def build_caption(info: dict) -> str:
    """
    يبني تنسيق HTML يشبه نص المنشور الأصلي: الناشر + الوصف/العنوان،
    مع رابط الفيديو كنص قابل للنقر بعنوان "رابط الفيديو" (Telegram Link format).
    يجب إرسال هذا النص مع parse_mode="HTML".
    """
    ...
    # نقصّ النص الخام أولًا قبل الترميز لتفادي قطع وسوم HTML في المنتصف
    ...
    try:
        ...
        if shutil.which("ffmpeg") is None:
            await status_msg.edit_text(
                ...
                "تأكد من رفع ملف nixpacks.toml إلى المستودع على GitHub وإعادة النشر."
            )
        else:
...
def check_ffmpeg():
    """يتحقق من وجود ffmpeg في السيرفر ويسجل تحذيرًا واضحًا إن لم يكن موجودًا.
    بدون ffmpeg، تحميل الصوت (وأحيانًا دمج الفيديو+الصوت) سيفشل."""
    if shutil.which("ffmpeg") is None:
        logger.warning(
            "⚠️ ffmpeg غير موجود على السيرفر! ميزة 'تحميل الصوت' لن تعمل. "
            "تأكد من رفع ملف nixpacks.toml إلى المستودع على GitHub وإعادة النشر على Railway."
        )
    else:
