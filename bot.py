...
def diagnose_download_error(error_text: str) -> str:
    """يترجم رسالة خطأ yt-dlp التقنية إلى سبب مفهوم بالعربي."""
    t = error_text.lower()
    if "private" in t:
        return "❌ هذا الفيديو خاص (Private) ولا يمكن الوصول إليه."
    if "unavailable" in t or "not available" in t or "removed" in t or "404" in t:
        return "❌ هذا الفيديو غير متوفر أو تم حذفه من المنصة."
    if "geo" in t or "not available in your country" in t or "region" in t:
        return "❌ هذا الفيديو محظور جغرافيًا في منطقة السيرفر."
    if "login" in t or "sign in" in t or "cookies" in t or "authentication" in t:
        return "❌ هذا المحتوى يتطلب تسجيل دخول لعرضه، ولا يمكن للبوت تحميله."
    if "unsupported url" in t or "no extractor" in t:
        return "❌ هذه المنصة أو نوع الرابط غير مدعوم حاليًا."
    if "429" in t or "too many requests" in t:
        return "❌ تم إرسال عدد كبير من الطلبات بوقت قصير، حاول بعد قليل."
    return (
        "❌ تعذر تحميل هذا الرابط.\n"
        "قد يكون السبب تحديثًا مؤقتًا في المنصة نفسها — جرّب مرة أخرى بعد قليل."
    )


def build_ydl_opts(
    out_dir: Path, unique_id: str, audio_only: bool = False, progress_hook=None, fallback: bool = False
) -> dict:
    ...
    if audio_only:
        ...
        # محاولة احتياطية مبسّطة تُستخدم فقط إذا فشلت الصيغة المفضلة أدناه
        opts["format"] = "best"
        opts["merge_output_format"] = "mp4"
        opts["postprocessor_args"] = ["-movflags", "+faststart"]
    else:
        # نفضّل كودك H.264/AAC لأنه الأوسع توافقًا مع مشغّل الفيديو داخل تليجرام
        opts["format"] = (
            "bestvideo[ext=mp4][vcodec^=avc1]+bestaudio[ext=m4a]/"
            "mp4/bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best"
        )
        opts["merge_output_format"] = "mp4"
        # يضمن وضع بيانات الفيديو في بداية الملف حتى يشتغل التشغيل الفوري (بدون هذا قد لا يعمل الفيديو بسلاسة)
        opts["postprocessor_args"] = ["-movflags", "+faststart"]
    return opts
...
def _run_ydl_download(opts: dict, url: str, out_dir: Path, unique_id: str, audio_only: bool) -> Path:
    with yt_dlp.YoutubeDL(opts) as ydl:
...
def download_media(url: str, out_dir: Path, audio_only: bool = False, progress_hook=None) -> Path:
    """
    يحمّل الوسائط. إن فشلت الصيغة المفضلة (يحدث أحيانًا لبعض الفيديوهات المحدّدة)
    يعيد المحاولة تلقائيًا بصيغة احتياطية أبسط قبل الاستسلام.
    """
    unique_id = uuid.uuid4().hex
    opts = build_ydl_opts(out_dir, unique_id, audio_only=audio_only, progress_hook=progress_hook)

    try:
        return _run_ydl_download(opts, url, out_dir, unique_id, audio_only)
    except yt_dlp.utils.DownloadError:
        if audio_only:
            raise  # لا حاجة لصيغة احتياطية للصوت، فقط ارفع الخطأ كما هو
        logger.warning(f"فشلت الصيغة المفضلة لـ {url}، تجربة صيغة احتياطية أبسط...")
        fb_unique_id = unique_id + "_fb"
        fallback_opts = build_ydl_opts(
            out_dir, fb_unique_id, audio_only=False, progress_hook=progress_hook, fallback=True
        )
        return _run_ydl_download(fallback_opts, url, out_dir, fb_unique_id, audio_only=False)
...
async def process_video_url(message, context: ContextTypes.DEFAULT_TYPE, url: str):
    async with DOWNLOAD_SEMAPHORE:
        ...
        try:
            ...
            await status_msg.edit_text(diagnose_download_error(str(e)))
        except Exception:
